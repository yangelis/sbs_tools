...
class SegmentBack:
    ...
    def get_tw_init(self) -> tuple[xt.TwissInit, np.ndarray]:
        ...
        betx_ini = self.mes.betx_free.BETX.loc[end_BPM]
        bety_ini = self.mes.bety_free.BETY.loc[end_BPM]
        alfx_ini = self.mes.betx_free.ALFX.loc[end_BPM]
        alfy_ini = self.mes.bety_free.ALFY.loc[end_BPM]

        f_ini_bbs = {}
        f_ini_bbs["f1001r"] = self.mes.coupling.F1001R.loc[end_BPM]
        f_ini_bbs["f1001i"] = self.mes.coupling.F1001I.loc[end_BPM]
        f_ini_bbs["f1010r"] = self.mes.coupling.F1010R.loc[end_BPM]
        f_ini_bbs["f1010i"] = self.mes.coupling.F1010I.loc[end_BPM]
        try:
            dx_ini = self.mes.data_dx.DX.loc[end_BPM]
            dy_ini = self.mes.data_dy.DY.loc[end_BPM]
            dpx_ini = self.mes.data_dx.DPX.loc[end_BPM]
            dpy_ini = self.mes.data_dy.DPY.loc[end_BPM]
        except:
            print("No dispersion measurements available")
    ...
    def get_phase_diffs(
        ...
        betx_ini = self.mes.betx.loc[bpms_x[1]].BETX.iloc[-1]
        bety_ini = self.mes.bety.loc[bpms_y[1]].BETY.iloc[-1]
        betx_ini_err = self.mes.betx.loc[bpms_x[1]].ERRBETX.iloc[-1]
        bety_ini_err = self.mes.bety.loc[bpms_y[1]].ERRBETY.iloc[-1]

        alfx_ini = self.mes.betx.loc[bpms_x[1]].ALFX.iloc[-1]
        alfy_ini = self.mes.bety.loc[bpms_y[1]].ALFY.iloc[-1]
        alfx_ini_err = self.mes.betx.loc[bpms_x[1]].ERRALFX.iloc[-1]
        alfy_ini_err = self.mes.bety.loc[bpms_y[1]].ERRALFY.iloc[-1]
